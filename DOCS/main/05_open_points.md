...
**Stato (agg. 2026-09-23):** trasporto via **AzCopy** ([[ADR-0023]]), **non** più SFTP — a tendere eseguito da **processi ODI** (owner: team). File sullo stesso storage ADLS. Cartelle **`YYYY/MM/DD`**, formato **CSV** (Parquet in futuro), convenzione `<source>-landing/{tabella}/YYYY/MM/DD/` già nei Bronze. **`landing_mode=external` DECISO** (un Volume managed non vede i file scritti da AzCopy) → impatta D3/[[ADR-0003]]; flip Terraform a valle del container.  
**Azione residua:** thread piattaforma **concluso (2026-09-23)** — architettura confermata; decisioni comunicate a Reply/Eddy: **container `logisticolanding`** (solo template Blob, no RG) + **SAS** per la scrittura (Read/Write/Create/Add/List) + **Access Connector/External Location** per la lettura. **Provisioning in corso** lato Reply → poi validare `--send` reale + flip `landing_mode=external`. Confermare con Foconi i nomi sorgente (`logistix`, `cdtdw`, `stat`). Backend `send_to_landing.py` in main (15 test, dry-run validato).
